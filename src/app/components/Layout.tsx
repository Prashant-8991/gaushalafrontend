...
function LayoutInner() {
  ...
  return (
    <div className="flex h-screen overflow-hidden" style={{ fontFamily: "'Poppins', sans-serif" }}>
      ...
      <aside
        className={`fixed lg:static inset-y-0 left-0 z-50 w-64 bg-sidebar text-sidebar-foreground transform transition-transform duration-300 ease-in-out ${sidebarOpen ? "translate-x-0" : "-translate-x-full lg:translate-x-0"
      ...
      >
        <div className="relative overflow-hidden">
          ...
          <div className="relative p-5 border-b border-sidebar-border">
            <div className="flex items-center gap-3">
              ...
              <div>
                {/* <h2 className="text-white tracking-wide font-semibold" style={{ fontSize: '1.1rem' }}>GauShala</h2> */}
                <p style={{ fontSize: '0.65rem' }} className="text-saffron-light/80 tracking-wider uppercase font-medium">Shree Somnath Trust Gaushala</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <nav className="flex-1 p-3 space-y-0.5 overflow-y-auto">
          {/* <p className="px-3 py-2 text-sidebar-foreground/30 uppercase tracking-widest text-[0.6rem] font-semibold">Main Menu</p> */}
          {navItems.map((item) => (
            <NavLink
              key={item.to}
              to={item.to}
            ...
            >
              <item.icon className={`w-5 h-5 transition-transform duration-200 group-hover:scale-110 ${navItems.indexOf(item) === 0 ? "group-[:not(.active)]:rotate-0" : ""}`} />
          ...
          {(user?.role === "admin" || user?.role === "manager") && (
            <>
              <div className="pt-4 pb-1">
                <p className="px-3 text-sidebar-foreground/30 uppercase tracking-widest text-[0.6rem] font-semibold">Administration</p>
              </div>
        ...
        <div className="p-3 border-t border-sidebar-border space-y-2">
          {user && (
            <div className="bg-white/5 backdrop-blur-sm rounded-xl p-3 flex items-center gap-3 group hover:bg-white/8 transition-colors">
              <div className="w-9 h-9 rounded-full bg-gradient-to-br from-saffron to-saffron-dark flex items-center justify-center shrink-0 shadow-lg shadow-saffron/20">
                <span className="text-white font-bold" style={{ fontSize: "0.7rem" }}>{user.initials}</span>
              </div>
              <div className="flex-1 min-w-0">
                <p className="text-sidebar-foreground font-medium truncate" style={{ fontSize: "0.75rem" }}>{user.displayName}</p>
                <span
          ...
          <div className="bg-white/5 rounded-xl p-3">
            <p style={{ fontSize: '0.7rem' }} className="text-sidebar-foreground/40">Somnath Temple Trust</p>
            <p style={{ fontSize: '0.7rem' }} className="text-saffron-light/70 font-medium">Gujarat, India</p>
          </div>
        </div>
      </aside>
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        <header className="bg-background/80 backdrop-blur-md border-b border-border px-4 lg:px-6 py-3 flex items-center justify-between shrink-0 sticky top-0 z-30">
          ...
          <div className="flex items-center gap-2">
            <span style={{ fontSize: '0.8rem' }} className="text-muted-foreground font-medium">
          ...
          <div className="flex items-center gap-3">
            <button
              ...
              title={cardTheme === "light" ? "Switch to dark mode" : "Switch to light mode"}
            >
              {cardTheme === "light" ? (
            ...
            {user && (
              <div className="flex items-center gap-2">
                <span
                  className={`hidden sm:inline px-2.5 py-0.5 rounded-full text-white font-medium capitalize shadow-sm ${ROLE_BADGE[user.role]}`}
                  style={{ fontSize: "0.65rem" }}
                >
                  {user.role}
                </span>
                <div className="w-9 h-9 rounded-full bg-gradient-to-br from-saffron to-saffron-dark flex items-center justify-center text-white font-bold shadow-lg shadow-saffron/20" style={{ fontSize: '0.7rem' }}>
                  {user.initials}
                </div>
              </div>
            )}
